...
export class ManagementSubjectTeacherController {
  ...
  static getSubjectTeacherById = asyncHandler(
    async (req: Request, res: Response) => {
      const schoolId = req.user?.schoolId;
      if (!schoolId) throw new AppError("Unauthorized.", 401);
      ...
      const assignment =
        await ManagementSubjectTeacherService.getSubjectTeacherById(
          schoolId,
          id,
        );
  ...
  static updateSubjectTeacher = asyncHandler(
    async (req: Request, res: Response) => {
      const schoolId = req.user?.schoolId;
      if (!schoolId) throw new AppError("Unauthorized.", 401);
      ...
      const assignment =
        await ManagementSubjectTeacherService.updateSubjectTeacher(
          schoolId,
          id,
          teacherId,
          classId,
